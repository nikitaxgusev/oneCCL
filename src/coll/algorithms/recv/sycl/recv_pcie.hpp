...
#endif
            const_cast<Recv*>(this)->runRecv(wireOff, tOff, workLeft);
        }
    }

private:
    // TODO: buffer plan and start point calc
    static size_t calcWorkSize(T* output, size_t output_sz) {
        ...
        auto octSize = divUp(output_sz, sizeof(message_t));

        // predicates granularity
        if (octSize * sizeof(message_t) != output_sz) {
